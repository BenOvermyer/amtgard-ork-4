...
class JsonServer
{
	...
	function __construct( $valid_classes = null, $valid_methods = null )
	{
		...
		$this->valid_method = is_array( $valid_methods ) ? $valid_methods : [ ];
		$this->map = [ ];
		$this->method = [ ];
		$this->definitions = [ ];
		$this->callback = [ ];
	...
	public function __call( $class, $arguments )
	{
		if ( count( $arguments ) == 2 ) {
			if ( is_callable( $arguments[ 1 ] ) ) {
				...
				if ( !isset( $this->definitions[ $class ] ) ) $this->definitions[ $class ] = [ ];
	...
	{
		$r = [ 'Result' => $result, 'Status' => $status, 'Code' => $status ? Errors::SUCCESS : $code ];
	...
	{
		return isset( $this->callback[ "$class/$method" ] ) ? $this->callback[ "$class/$method" ] : [ $class, $method ];
	}
	...
	private function run_call( $class, $method, $args )
	{
		...
		if ( $args === false ) {
			...
			$instance = new $c();
			$result = call_user_func_array( [ $instance, $m ], $args );
			if ( is_array( $result ) ) {
			...
			return [ 'Result' => $result, 'Status' => true, 'Code' => Errors::SUCCESS ];
		}
	}

	private function error( $code, $message, $trace = null )
	{
		return [ 'Result' => null, 'Status' => false, 'Code' => Errors::CLASS_ERROR, 'ClassCode' => $code, 'ErrorMessage' => $message, 'Trace' => $trace ];
	}

	private function general_error( $line = 0 )
	{
		return [ 'Result' => null, 'Status' => false, 'Code' => Errors::GENERAL_ERROR, 'Details' => 'A general error occured on line ' . $line ];
	}
