...
class Reports extends Model
{
	...
	{
		return $this->Report->TournamentReport( [
	...
	function get_attendance_summary( $type, $id, $period, $num_periods )
	{
		...
		if ( 'All' == $period ) {
			$r = $this->Report->AttendanceSummary( [ 'KingdomId' => $type == 'Kingdom' ? $id : null, 'ParkId' => $type == 'Park' ? $id : null, 'PrincipalityId' => $type == 'Principality' ? $id : null, 'EventId' => $type == 'Event' ? $id : null, 'ReportFromDate' => date( 'Y-m-d' ), 'Periods' => 360, 'PerWeeks' => 0, 'PerMonths' => 1 ] );
		} else {
			$r = $this->Report->AttendanceSummary( [ 'KingdomId' => $type == 'Kingdom' ? $id : null, 'ParkId' => $type == 'Park' ? $id : null, 'PrincipalityId' => $type == 'Principality' ? $id : null, 'EventId' => $type == 'Event' ? $id : null, 'ReportFromDate' => date( 'Y-m-d' ), 'Periods' => $num_periods, 'PerWeeks' => $period == 'Weeks' ? 1 : 0, 'PerMonths' => $period == 'Months' ? 1 : 0 ] );
		}
		return $r;
	}
	...
	function get_periodical_summary( $type, $id, $period, $num_periods, $by_period )
	{
		...
		if ( 'All' == $period ) {
			$r = $this->Report->AttendanceSummary( [ 'KingdomId' => $type == 'Kingdom' ? $id : null, 'ParkId' => $type == 'Park' ? $id : null, 'PrincipalityId' => $type == 'Principality' ? $id : null, 'EventId' => $type == 'Event' ? $id : null, 'ReportFromDate' => date( 'Y-m-d' ), 'Periods' => 360, 'PerWeeks' => 0, 'PerMonths' => 1, 'ByPeriod' => 'week' ] );
		} else {
			$r = $this->Report->AttendanceSummary( [ 'KingdomId' => $type == 'Kingdom' ? $id : null, 'ParkId' => $type == 'Park' ? $id : null, 'PrincipalityId' => $type == 'Principality' ? $id : null, 'EventId' => $type == 'Event' ? $id : null, 'ReportFromDate' => date( 'Y-m-d' ), 'Periods' => $num_periods, 'PerWeeks' => $period == 'Weeks' ? 1 : 0, 'PerMonths' => $period == 'Months' ? 1 : 0, 'ByPeriod' => 'week' ] );
		}
		return $r;
	}
