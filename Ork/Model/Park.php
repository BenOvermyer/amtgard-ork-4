...
class Park extends Model
{
	...
	function get_park_details( $park_id )
	{
		...
		return [ 'ParkConfiguration' => $this->Park->GetParkConfiguration( $request ),
				 'ParkDays'          => $this->Park->GetParkDays( $request ),
				 'ParkInfo'          => $this->Park->GetParkDetails( $request ),
				 'Heraldry'          => $this->Heraldry->GetHeraldryUrl( [ 'Type' => 'Park', 'Id' => $park_id ] ) ];
	}

	function get_officers( $park_id )
	{
		$r = $this->Park->GetOfficers( [ 'ParkId' => $park_id ] );
	...
	{
		return $this->Park->GetParkShortInfo( [ 'ParkId' => $park_id ] );
	}
	...
	{
		return $this->Park->GetParkDays( [ 'ParkId' => $park_id ] );
	}
