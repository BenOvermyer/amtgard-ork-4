...
class Kingdom extends Model
{
	...
	{
		return $this->Kingdom->GetPrincipalities( [ 'KingdomId' => $kingdom_id ] );
	}
	...
	{
		$r = $this->Kingdom->GetParks( [ 'KingdomId' => $kingdom_id ] );
		$pt = $this->Kingdom->GetKingdomParkTitles( [ 'KingdomId' => $kingdom_id ] );
	...
	{
		$r = $this->Kingdom->GetOfficers( [ 'KingdomId' => $kingdom_id ] );
	...
	{
		return [
			'Info'        => $this->Kingdom->GetKingdomShortInfo( [ 'KingdomId' => $kingdom_id ] ),
			'HeraldryUrl' => $this->Heraldry->GetHeraldryUrl( [ 'Type' => 'Kingdom', 'Id' => $kingdom_id ] ),
		];
	}

	function get_kingdom_details( $kingdom_id )
	{
		$r = $this->Kingdom->GetKingdomDetails( [ 'KingdomId' => $kingdom_id ] );
		$r[ 'Heraldry' ] = $this->Heraldry->GetHeraldryUrl( [ 'Type' => 'Kingdom', 'Id' => $kingdom_id ] );
		return $r;
	}
