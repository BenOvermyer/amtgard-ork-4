...
class Kingdom extends Ork3
{
	...
	public function GetAwardList( $request )
	{
		...
		if ( $r !== false && $r->size() > 0 ) {
			$response[ 'Awards' ] = [ ];
			do {
				$response[ 'Awards' ][ $r->kingdomaward_id ] = [
	...
	*/
	...
	public function GetKingdomDetails( $request )
	{
		...
		if ( $request[ 'KingdomId' ] > 0 && $this->kingdom->find() ) {
			...
			$response[ 'Awards' ] = $this->GetAwardList( [ 'IsLadder' => 'Both', 'IsTitle' => 'Both', 'KingdomId' => $request[ 'KingdomId' ] ] );
		} else {
			$response[ 'Status' ] = InvalidParameter();
		}
		return $response;
	...
	public function CreateKingdom( $request )
	{
		...
		if ( ( $mundane_id = Ork3::$Lib->authorization->IsAuthorized( $request[ 'Token' ] ) ) > 0
			...
			$c->add_config( $mundane_id, CFG_KINGDOM, 'fixed', $this->kingdom->kingdom_id, 'AveragePeriod',
				[ 'Type' => $request[ 'AttendancePeriodType' ], 'Period' => $request[ 'AttendancePeriod' ] ], 1, [ 'Type' => [ 'month', 'week' ] ] );
			...
			$c->add_config( $mundane_id, CFG_KINGDOM, 'fixed', $this->kingdom->kingdom_id, 'DuesPeriod', [ 'Type' => $request[ 'DuesPeriodType' ], 'Period' => $request[ 'DuesPeriod' ] ], 1, [ 'Type' => [ 'month', 'week' ] ] );
	...
	public function GetPrincipalities( $request )
	{
		...
		$result = [ 'Status' => Success(), 'Principalities' => [ ] ];
		if ( $this->kingdom->find() ) {
			do {
		...
		if ( $r !== false && $r->size() > 0 ) {
			...
			do {
				$response[ 'Parks' ][] = [
					...
					'ParentOf'     => $r->is_principality == 1 ? Ork3::$Lib->park->GetParks( [ 'ParkId' => $r->park_id, 'Stack' => [ $r->park_id ] ] ) : null,
				];
			} while ( $r->next() );
		} else {
			$response[ 'Status' ] = InvalidParameter();
	...
	public function GetKingdoms( $request )
	{
		...
		if ( $this->kingdom->find() ) do {
			...
			$response[ 'Kingdoms' ][ $this->kingdom->kingdom_id ] = [
				'KingdomId'    => $this->kingdom->kingdom_id,
