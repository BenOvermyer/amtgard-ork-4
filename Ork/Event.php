...
class Event extends Ork3
{
	...
	public function GetEvent( $request )
	{
		...
		if ( valid_id( $request[ 'EventId' ] ) && $this->event->find() ) {
			...
			$response[ 'HeraldryUrl' ] = $this->event->has_heraldry ? Ork3::$Lib->heraldry->GetHeraldryUrl( [ 'Type' => 'Event', 'Id' => $request[ 'EventId' ] ] ) : Ork3::$Lib->heraldry->GetHeraldryUrl( [ 'Type' => 'Event', 'Id' => 0 ] );
	...
	public function GetEventDetail( $request )
	{
		...
		if ( valid_id( $request[ 'EventCalendarDetailId' ] ) && $this->detail->find() ) {
			...
			logtrace( 'Event->GetEventDetail()', [ $request, $this->detail, $this->detail->lastSql() ] );
			$response[ 'Status' ] = InvalidParameter();
		}
		return $response;
	}
	...
	public function GetEventDetails( $request )
	{
		...
		$response = [ ];
		if ( valid_id( $request[ 'EventId' ] ) && $this->detail->find( [ 'event_start DESC' ], 'AND', ( $request[ 'Current' ] ? 1 : null ) ) ) {
			$response[ 'CalendarEventDetails' ] = [ ];
			do {
				$nr = [ ];
				$nr[ 'EventCalendarDetailId' ] = $this->detail->event_calendardetail_id;
	...
	public function SetCurrent( $request )
	{
		...
		if ( $mundane_id > 0 && Ork3::$Lib->authorization->HasAuthority( $mundane_id, AUTH_EVENT, $event_id, AUTH_CREATE ) ) {
			if ( valid_id( $request[ 'EventCalendarDetailId' ] ) && $this->detail->find() ) {
				...
				logtrace( "SetCurrent()", [ $request, $this->detail->lastSql() ] );
	...
	public function SetEventDetails( $request )
	{
		...
		if ( valid_id( $mundane_id ) && Ork3::$Lib->authorization->HasAuthority( $mundane_id, AUTH_EVENT, $request[ 'EventId' ], AUTH_EDIT ) ) {
			...
			$this->detail->event_calendardetail_id = $request[ 'EventCalendarDetailId' ];
			if ( valid_id( $request[ 'EventCalendarDetailId' ] ) && $this->detail->find() ) {
				if ( Ork3::$Lib->attendance->HasAttendance( [ 'Filter' => 'Event', 'Value' => $request[ 'EventCalendarDetailId' ] ] ) )
				...
				if ( valid_id( $request[ 'Current' ] ) ) {
					logtrace( "SetEventDetails", [ 'Token' => $request[ 'Token' ], 'EventId' => $request[ 'EventId' ], 'EventCalendarDetailId' => $request[ 'EventCalendarDetailId' ], 'Current' => 1 ] );
					$this->SetCurrent( [ 'Token' => $request[ 'Token' ], 'EventCalendarDetailId' => $request[ 'EventCalendarDetailId' ], 'Current' => 1 ] );
				}
				logtrace( 'SetEventDetails', $request );
			} else {
				return InvalidParameter( '' );
		...
		if ( valid_id( $mundane_id ) && Ork3::$Lib->authorization->HasAuthority( $mundane_id, AUTH_EVENT, $request[ 'EventId' ], AUTH_EDIT ) ) {
			...
			if ( valid_id( $request[ 'EventId' ] ) && $this->event->find() ) {
				...
				if ( is_numeric( $request[ 'ParkId' ] ) ) {
					$this->event->park_id = $request[ 'ParkId' ];
					$p = Ork3::$Lib->park->GetParkShortInfo( [ 'ParkId' => $request[ 'ParkId' ] ] );
				...
				$this->event->save();
				Ork3::$Lib->heraldry->SetEventHeraldry( $request );
				logtrace( "SetEvent", [ $request, $this->event ] );
				return Success();
			} else {
