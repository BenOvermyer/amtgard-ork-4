...
class Admin extends Controller
{
	...
	public function unit( $unit_id )
	{
			...
			if ( !isset( $this->session->user_id ) ) {
				...
				switch ( $this->request->Action ) {
					...
					case 'retire':
						...
						break;
					case 'remove':
						$r = $this->Unit->remove_unit_member( [
							'Token'         => $this->session->token,
							'UnitMundaneId' => $this->request->Admin_unit->UnitMundaneId,
							'UnitId'        => $unit_id,
						] );
		...
		$this->data[ 'menu' ][ 'admin' ] = [ 'url' => UIR . "Admin/unit/$unit_id", 'display' => 'Admin' ];
		$this->data[ 'menu' ][ 'unit' ] = [ 'url' => UIR . "Unit/index/$unit_id", 'display' => $this->data[ 'Unit' ][ 'Details' ][ 'Unit' ][ 'Name' ] ];
	}
			...
			if ( !isset( $this->session->user_id ) ) {
				...
				switch ( $this->request->Action ) {
					case 'details':
						...
						$Status = $this->Park->SetParkDetails( [
							'Token'            => $this->session->token,
							'ParkId'           => $park_id,
					...
					case 'addparkday':
						...
						break;
					case 'delete':
						$Status = $this->Park->delete_park_day( [
							'Token'     => $this->session->token,
		...
		if ( strlen( $post ) > 0 ) {
			...
			if ( !isset( $this->session->user_id ) ) {
				...
				if ( valid_id( $this->request->Admin_setofficers->MonarchId ) )
					$officers[ 'Monarch' ] = [ 'MundaneId' => $this->request->Admin_setofficers->MonarchId, 'Role' => 'Monarch' ];
				if ( valid_id( $this->request->Admin_setofficers->Prime_MinisterId ) )
					$officers[ 'Prime_Minister' ] = [ 'MundaneId' => $this->request->Admin_setofficers->Prime_MinisterId, 'Role' => 'Prime Minister' ];
				if ( valid_id( $this->request->Admin_setofficers->RegentId ) )
					$officers[ 'Regent' ] = [ 'MundaneId' => $this->request->Admin_setofficers->RegentId, 'Role' => 'Regent' ];
				if ( valid_id( $this->request->Admin_setofficers->ChampionId ) )
					$officers[ 'Champion' ] = [ 'MundaneId' => $this->request->Admin_setofficers->ChampionId, 'Role' => 'Champion' ];
	...
	public function event( $p )
	{
		...
		if ( strlen( $post ) > 0 ) {
			...
			if ( !isset( $this->session->user_id ) ) {
				...
				logtrace( "index($p)", [ $h_im, $h_imdata ] );
				$edit = [
					'Token'            => $this->session->token,
					'EventId'          => $event_id,
		...
		$this->data[ 'menu' ][ 'event' ] = [ 'url' => UIR . 'Event/index/' . $event_id, 'display' => $this->data[ 'EventDetails' ][ 'Name' ] ];
		$this->data[ 'menu' ][ 'admin' ] = [ 'url' => UIR . 'Admin/event/' . $event_id, 'display' => 'Admin' ];
	}
	...
	public function manageevent( $post = null )
	{
		...
		if ( valid_id( $this->request->UnitId ) || valid_id( $this->request->Admin_manageevent->CreateUnitId ) ) {
			...
			if ( valid_id( $this->data[ 'CreateUnitId' ] ) ) {
				...
				$this->data[ 'menu' ][ 'player' ] = [ 'url' => UIR . "Unit/index/{$this->data[CreateUnitId]}", 'display' => $unit[ 'Unit' ][ 'Name' ] ];
				$this->data[ 'menu' ][ 'admin' ] = [ 'url' => UIR . "Admin/unit/{$this->data[CreateUnitId]}", 'display' => 'Admin' ];
			}
		}
		if ( valid_id( $this->request->MundaneId ) || valid_id( $this->request->Admin_manageevent->CreateMundaneId ) ) {
			...
			if ( valid_id( $this->data[ 'CreateMundaneId' ] ) ) {
				...
				$this->data[ 'menu' ][ 'player' ] = [ 'url' => UIR . "Player/index/{$this->data[CreateMundaneId]}", 'display' => $player[ 'Persona' ] ];
				$this->data[ 'menu' ][ 'admin' ] = [ 'url' => UIR . "Admin/player/{$this->data[CreateMundaneId]}", 'display' => 'Admin' ];
			}
		}
	...
	public function createevent( $post = null )
	{
		$this->load_model( 'Event' );
		if ( valid_id( $this->request->UnitId ) || valid_id( $this->request->Admin_manageevent->CreateUnitId ) ) {
			$this->data[ 'Events_list' ] = $this->Event->GetEvents( [ 'UnitId' => valid_id( $this->request->Admin_manageevent->UnitId ) ? $this->request->Admin_manageevent->UnitId : $this->request->UnitId, 'LimitTo' => true ] );
			...
			$this->data[ 'Events_list' ] = $this->Event->GetEvents( [ 'MundaneId' => valid_id( $this->request->Admin_manageevent->MundaneId ) ? $this->request->Admin_manageevent->MundaneId : $this->request->MundaneId, 'LimitTo' => true ] );
			$this->data[ 'EventIdSelector' ] = 'MundaneId';
		} else if ( isset( $this->session->park_id ) && valid_id( $this->session->park_id ) ) {
			$this->data[ 'Events_list' ] = $this->Event->GetEvents( [ 'ParkId' => $this->session->park_id, 'LimitTo' => false ] );
			$this->data[ 'EventIdSelector' ] = 'ParkId';
		} else if ( isset( $this->session->kingdom_id ) && valid_id( $this->session->kingdom_id ) ) {
			$this->data[ 'Events_list' ] = $this->Event->GetEvents( [ 'KingdomId' => $this->session->kingdom_id, 'LimitTo' => true ] );
			$this->data[ 'EventIdSelector' ] = 'KingdomId';
		} else {
			$this->data[ 'Events_list' ] = [ ];
		}
	}
				...
				switch ( $action ) {
					...
					case 'update':
						if ( $this->request->RemoveDues == 'Revoke Dues' ) {
							...
							$this->Treasury->RemoveLastDuesPaid( [
								'MundaneId' => $id,
								'Token'     => $this->session->token,
							] );
						}
						...
						if ( $this->request->Update == 'Update Details' ) {
							if ( valid_id( $this->request->Admin_player->DuesSemesters ) ) {
								$this->load_model( 'Treasury' );
								$duespaid = $this->Treasury->DuesPaidToPark( [
									'Token'           => $this->session->token,
									'MundaneId'       => $id,
					...
					case 'deleteaward':
						...
						break;
					case 'updateaward':
						$r = $this->Player->update_player_award( [
							'Token'       => $this->session->token,
							'AwardsId'    => $roastbeef,
							'RecipientId' => $id,
	...
	public function player_bak( $mundane_id )
	{
		...
		$this->data[ 'menu' ][ 'admin' ] = [ 'url' => UIR . "Admin/player/$mundane_id", 'display' => 'Admin' ];
		$this->data[ 'menu' ][ 'player' ] = [ 'url' => UIR . "Player/index/$mundane_id", 'display' => $this->data[ 'Player' ][ 'Persona' ] ];
	}
	...
	public function createkingdom( $post = null )
	{
		...
		$this->data[ 'AttendancePeriodType_options' ] = [ 'month' => 'Month', 'week' => 'Week' ];
		$this->data[ 'DuesPeriodType_options' ] = [ 'month' => 'Month', 'week' => 'Week' ];
	...
	public function editkingdom( $id )
	{
			...
			if ( !isset( $this->session->user_id ) ) {
				...
				switch ( $this->request->Action ) {
					...
					case 'deletetitle':
						...
						break;
					case 'awards':
						$KAwards = $this->Kingdom->GetAwardList( [ 'KingdomId' => $id, 'IsLadder' => 'Either', 'IsTitle' => 'Either' ] );
		...
		$this->load_model( 'Award' );
		$this->data[ 'Canonical_awards' ] = $this->Award->GetAwardList( [ ] );
