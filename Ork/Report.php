...
class Report extends Ork3
{
	...
	{
		$reports = [ 'Status' => Success(), 'Reports' => [ ] ];
		$reports = [
			'HeraldryReport'           => [
				'Name'        => 'HeraldryReport',
				...
				'Parameters'  => [
					[
						...
						'Definition' => [
							'Type'        => 'Fixed',
							'Parameters'  => [ 'kingdom', 'park', 'unit', 'mundane' ],
		...
		if ( $$table->find() ) {
			...
			do {
				$response[] = [
					...
					'HeraldryUrl' => Ork3::$Lib->heraldry->GetHeraldryUrl( [ 'Type' => ( $request[ 'Type' ] == 'Mundane' ? 'Player' : $request[ 'Type' ] ), 'Id' => $$table->$table_id ] ),
	...
	public function UnitSummary( $request )
	{
		...
		$sql = "select distinct u.*, m.*, count(um.mundane_id) as member_count
				...
				order by u.name";
		$r = $this->db->query( $sql );
		logtrace( "Unit Summary", [ $request, $sql ] );
		$response = [ 'Status' => Success(), 'Units' => [ ] ];
		if ( $r === false ) {
			$response[ 'Status' ] = InvalidParameter();
		} else if ( $r->size() > 0 ) {
			do {
	...
	public function AttendanceSummary( $request )
	{
		...
		*/
		...
		logtrace( "Report->AttendanceSummary()", [ $this->db->lastSql, $request ] );
		return $response;
	}
	...
	public function AttendanceForEvent( $request )
	{
		...
		logtrace( 'AttendanceForEvent', [ $request, $sql ] );
	...
	public function AttendanceForDate( $request )
	{
		...
		logtrace( 'AttendanceForDate', [ $request, $sql ] );
	...
	public function GetPlayerRoster( $request )
	{
		...
		switch ( $request[ 'Type' ] ) {
			...
			case AUTH_EVENT:
				...
				$select_list = [ 'um.role', 'um.title', 'um.active' ];
				$order_by = "um.unit_id";
			...
			case AUTH_UNIT:
				...
				$select_list = [ 'um.role', 'um.title', 'um.active', 'um.role as unit_role', 'um.title as unit_title', 'um.unit_mundane_id' ];
				$order_by = "um.unit_id";
		...
		$select_list = array_merge( $select_list, [ 'm.mundane_id', 'm.persona', 'm.park_id', 'm.kingdom_id', 'm.restricted', 'm.waivered', 'm.given_name', 'm.surname', 'm.other_name', 'p.name as park_name', 'k.name as kingdom_name', 'm.penalty_box' ] );
		...
		";
		logtrace( 'GetPlayerRoster()', [ $sql, $restrict_clause ] );
		$r = $this->db->query( $sql );
	...
	public function GetKingdomParkAverages( $request )
	{
		...
		$sql = "select 
					...
					group by park_id
					order by name";
		logtrace( 'Report: GetKingdomParkAverages', [ $request, $sql ] );
		$r = $this->db->query( $sql );
		$response = [
			'Status'                     => Success(),
		...
		if ( $r === false ) {
			...
			$report = [ ];
			do {
				$report[] = [ 'AttendanceCount' => $r->attendance_count, 'ParkId' => $r->park_id, 'ParkName' => $r->name, 'Title' => $r->title, 'ParkTitleId' => $r->parktitle_id ];
			} while ( $r->next() );
	...
	public function GetActiveKingdomsSummary( $request = null )
	{
		...
		$sql = "SELECT k.name, k.kingdom_id, k.parent_kingdom_id, pcount.park_count, ifnull(attendance_count,0) attendance, ifnull(monthly_attendance_count,0) monthly, ifnull(activeparks.parkcount,0) active_parks 
					...
					where active = 'Active'
                    order by k.name";
		logtrace( 'Report: GetActiveKingdomsSummary', [ $request, $sql ] );
		$r = $this->db->query( $sql );
		$report = [ ];
		do {
			$report[] = [ 'KingdomName'    => $r->name, 'ParentKingdomId' => $r->parent_kingdom_id,
	...
	public function GetActivePlayers( $request )
	{
		...
		logtrace( 'Report: GetActivePlayers', [ $request, $sql ] );
		$r = $this->db->query( $sql );
		$report = [ ];
