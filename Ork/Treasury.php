...
class Treasury extends Ork3
{
	...
	public function DuesPaidToPark( $request )
	{
		...
		if ( ( $mundane_id = Ork3::$Lib->authorization->IsAuthorized( $request[ 'Token' ] ) ) > 0
			...
			$park_info = Ork3::$Lib->park->GetParkShortInfo( [ 'ParkId' => $player[ 'ParkId' ] ] );
	...
	public function create_accounts( $mundane_id, $type, $id, $kingdom_id = 0 )
	{
		...
		$c = new Common();
		$c->add_config( $mundane_id, ucfirst( $type ), 'mixed', $id, 'AccountPointers', [
			'Imbalance'        => $imbalance,
			'Equity'           => $equity,
			'Asset'            => $asset,
			'Cash'             => $cash,
	...
	public function has_account_authority( $mundane_id, $account_id )
	{
		...
		logtrace( 'has_account_authority', [ $mundane_id, $account_id, $type, $id ] );
	...
	{
		logtrace( 'record_transaction', [ $trn, $splits, $trx_date ] );
		...
		foreach ( $splits as $s => $split ) {
			...
			if ( !$this->account->find() ) {
				return InvalidParameter( print_r( [ $split[ 'AccountId' ], true ] ) );
			} else {
				$splits[ $s ][ 'AccountType' ] = $this->account->type;
			}
		}
		if ( !$authority ) return NoAuthorization( print_r( [ $trn[ 'RecordedBy' ], $split[ 'AccountId' ] ], true ) );
	...
	function normalize_sign( $t, $amt )
	{
		switch ( $t ) {
			...
			case TreasuryAccountType::Draw:
				return $amt >= 0 ? [ TreasuryDrCr::Dr, $amt ] : [ TreasuryDrCr::Cr, -$amt ];
			...
			case TreasuryAccountType::Imbalance:
				return $amt >= 0 ? [ TreasuryDrCr::Cr, $amt ] : [ TreasuryDrCr::Dr, -$amt ];
		}
	}
