...
class Game extends Ork3
{
		...
		if ( ( $mundane_id = Ork3::$Lib->authorization->IsAuthorized( $request[ 'Token' ] ) ) > 0 ) {
			switch ( $request[ 'Type' ] ) {
				case 'flag-capture':
					...
					break;
				default:
					return Success( $this->create_game( $request[ 'Name' ], 'custom', $mundane_id, $request[ 'Configuration' ], [ ] ) );
					break;
			}
		} else {
	...
	public function AddObjective( $request )
	{
		...
		echo "pass security";
		$game = $this->GetGame( [ 'GameId' => $request[ 'GameId' ] ] );
		print_r( $game );
	...
	private function create_game( $name, $type, $mundane_id, $configuration, $state )
	{
		...
		$this->game->save();
		return [ 'GameId' => $this->game->game_id, 'Code' => $this->game->code ];
	}
	...
	{
		return $this->add_objective( $game_id, $name, [
			'TeamData' => [ ],
		] );
		return 0;
	}
	...
	{
		$game = $this->GetGame( [ 'GameId' => $game_id ] );
		$obj = $this->GetObjective( [ 'ObjectiveId' => $objective_id ] );
		if ( count( $game ) == 0 ) return false;

		$state_source = [ 'Game' => $game, 'Objective' => $obj, 'State' => $state ];
		$ret_state = false;
		/*
			Inputs
			Objective
		...
		*/
		...
		---------------------------------------------------- */
		$basic_ctf = [ 'PointOnTarget' => true ];
		$start_resume_game = array_merge( $basic_ctf, [ 'Objective' => 0, 'RunStatus' => 'start' ] );
		...
		$hold_game = array_merge( $basic_ctf, [ 'Objective' => 0, 'RunStatus' => 'hold', 'GameStatus' => 'running' ] );
		...
		$flag_captured = array_merge( $basic_ctf, [ 'Objective' => 1, 'PointOnTarget' => true, 'GameStatus' => 'running' ] );
